/**
 * @Author: liuxin
 * @Date:   2022-10-06 16:01:50
 * @Last Modified by:   liuxin
 * @Last Modified time: 2022-10-26 22:31:22
 */

// The impact of class member variables on performance
#include <iostream>
#include <chrono>
#include <vector>
...
struct A
{
    ...
    __attribute_noinline__ void sum1(const int* a, int count) {
        m_total = 0;
    ...
    __attribute_noinline__ void sum2(const int *a, int count) {
        int total = m_total = 0;
        for (int i = 0; i < count; i++) {
            total += a[i];
        }
